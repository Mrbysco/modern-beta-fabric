...
public class BetaFancyOakFeature extends Feature<NoneFeatureConfiguration> {
	...
	private static final BlockState OAK_LEAVES = Blocks.OAK_LEAVES.defaultBlockState().setValue(LeavesBlock.DISTANCE, 1);
	...
	public BetaFancyOakFeature(Codec<NoneFeatureConfiguration> configCodec) {
		super(configCodec);
	}

	@Override
	public final boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		WorldGenLevel world = context.level();
		BlockPos pos = context.origin();
		RandomSource random = context.random();
		RandomSource treeRandom = new SingleThreadedRandomSource(random.nextLong());
	...
	private boolean canGenerate(WorldGenLevel world, BlockPos basePos, TreeInfo treeInfo) {
	...
	private void initializeTree(WorldGenLevel world, BlockPos basePos, RandomSource random, TreeInfo treeInfo) {
		...
		if (DIRT_REPLACEABLE.contains(world.getBlockState(mutable.set(basePos.getX(), basePos.getY() - 1, basePos.getZ())))) {
			world.setBlock(mutable, BlockStates.DIRT, 0);
		}
	...
	private void placeFoliageBlobs(WorldGenLevel world, BlockPos basePos, TreeInfo treeInfo) {
		int curBlob = 0;
	...
	private void placeTreeTrunk(WorldGenLevel world, BlockPos basePos, TreeInfo treeInfo) {
		int x = basePos.getX();
		int z = basePos.getZ();
	...
	private void placeTreeBranches(WorldGenLevel world, BlockPos basePos, TreeInfo treeInfo) {
		int curBranch = 0;
	...
	private void placeBranch(WorldGenLevel world, int[] startPos, int[] endPos, BlockState state) {
		BlockPos.MutableBlockPos mutable = new BlockPos.MutableBlockPos();
		...
		if (distance[longestSideNdx] != 0) {
			...
			while (offset != endOffset) {
				pos[longestSideNdx] = Mth.floor((startPos[longestSideNdx] + offset) + 0.5D);
				pos[ndx0] = Mth.floor(startPos[ndx0] + offset * branchStep0 + 0.5D);
				pos[ndx1] = Mth.floor(startPos[ndx1] + offset * branchStep1 + 0.5D);

				if (getLogAxis)
					state = state.trySetValue(RotatedPillarBlock.AXIS, this.getLogAxis(startPos, pos));

				world.setBlock(mutable.set(pos[0], pos[1], pos[2]), state, 19);
	...
	private void placeLayer(WorldGenLevel world, int x, int y, int z, float radius, byte axis, BlockState state) {
		BlockPos.MutableBlockPos mutable = new BlockPos.MutableBlockPos();
	...
	private void placeFoliageBlob(WorldGenLevel world, int x, int y, int z) {
		int curY = y;
	...
	private int getBranchLength(WorldGenLevel world, int[] startPos, int[] endPos) {
		BlockPos.MutableBlockPos mutable = new BlockPos.MutableBlockPos();
		...
		if (distance[longestSideNdx] != 0) {
			...
			while (offset != endOffset) {
				...
				pos[ndx0] = Mth.floor(startPos[ndx0] + offset * branchStep0);
				pos[ndx1] = Mth.floor(startPos[ndx1] + offset * branchStep1);
